...
function initPlanPanel() {
    ...
    // --- Helpers de date et de semaines ISO ---
    ...
    function recalc(){
        ...
        let ytd = 0;
        const targetMonth = view === 'mois' ? mSel : 12;
        if(mode === 'auto'){
             for(let mm=0; mm < targetMonth; mm++) { ytd += sumAnnualInMonth(prevData, ySel, mm); }
        } else {
             for(let mm=1; mm<=targetMonth; mm++){ const ymx = `${ySel}-${String(mm).padStart(2,'0')}`; ytd += loadArr('month', ymx).reduce((s,x)=>s+x,0); }
        }
