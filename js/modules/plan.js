// Contenu Autonome et Corrigé pour js/modules/plan.js
...
function initPlanPanel() {
    ...
    function renderWeeks(){
        ...
        els.weeks.innerHTML = wdefs.map((w,idx)=>{
            ...
            return `<div class="week-row" data-idx="${idx}"><div class="wk-label"><strong>S${String(w.isoWeek).padStart(2,'0')}</strong><small>${w.start} → ${w.end}</small></div>${input}${goalCheckHTML}</div>`;
        }).join('');
        if(mode!=='auto'){
    ...
    function recalc(){
        ...
        if (autoOn) {
            ...
            els.mgoal.style.opacity = '1'; els.mgoal.style.background = 'var(--surface)'; els.mgoal.style.borderStyle = 'dashed'; els.mgoal.style.cursor = 'not-allowed';
        } else if (els.mgoal) {
            els.mgoal.disabled = false;
            els.mgoal.style.opacity = '1'; els.mgoal.style.background = 'var(--elev)'; els.mgoal.style.borderStyle = 'solid'; els.mgoal.style.cursor = 'text';
        }
