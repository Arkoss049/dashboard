// Contenu mis à jour pour js/modules/plan.js
...
function initPlanPanel() {
 ...
 // --- Helpers et Gestion des Données (inchangé) ---
 ...
 function sumAnnualInMonth(data, year, month){ const ym = `${year}-${String(month+1).padStart(2,'0')}`; return round0(data.reduce((s,d)=> s + ((d.date||'').startsWith(ym) ? (Number(r.annual)||0) : 0), 0)); }
 ...
 // --- Initialisation de l'UI (inchangé) ---
 ...
 // NOUVEAU : Calcul de l'objectif hebdomadaire
 ...
 // NOUVEAU : On doit relancer le rendu des semaines après un recalcul pour mettre à jour les coches
 renderWeeks();
 }
 ...
 els.month.addEventListener('change', () => { localStorage.setItem(k('state','month'), els.month.value); if (!els.autoMGoalToggle.checked) { els.mgoal.value = loadVal('goal', els.month.value) || ''; } renderWeeks(); recalc(); });
