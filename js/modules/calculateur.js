function initCalculateurPanel() {
    ...
    const $ = sel => root.querySelector(sel); const SAVE_PREFIX = 'v8.calc.';
    ...
    function restoreAll(){ root.querySelectorAll('[data-save="1"]').forEach(el => { const key = SAVE_PREFIX + el.className.split(' ')[0]; const val = localStorage.getItem(key); if(val!=null){ el.value = val; } el.addEventListener('input', ()=>localStorage.setItem(key, el.value)); }); }
    ...
    restoreAll(); const savedProfile = localStorage.getItem(SAVE_PREFIX+'profile') || 'confirme'; els.profile.value = savedProfile; applyProfile(savedProfile);
    els.profile.addEventListener('change', (e)=> applyProfile(e.target.value)); els.calc.addEventListener('click', compute);
    els.pullPrev.addEventListener('click', () => { const prevTotal = JSON.parse(localStorage.getItem('prev.data')||'[]').reduce((s,d)=> s + (Number(d.annual)||0), 0); els.ca_prev.value = Math.round(prevTotal); compute(); });
    els.clearBtn.addEventListener('click', () => { if(confirm('Réinitialiser ?')){ root.querySelectorAll('[data-save="1"]').forEach(el => { localStorage.removeItem(SAVE_PREFIX + el.className.split(' ')[0]); }); applyProfile('confirme'); }});
    compute(); 
}
