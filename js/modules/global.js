function initGlobalPanel() {
    ...
    function fillFromLast(){
        ...
        if(!raw){ alert('Aucune simulation trouvée. Allez dans l’onglet Calculateur, vérifiez les chiffres et cliquez sur "Calculer la Prime".'); return; }
        const s = JSON.parse(raw);
        const roGlobal = (s.roPrev + s.roERc + s.roERe) / 3;
        els.ro_global.textContent = `${roGlobal.toFixed(1)}%`; setBadge(els.ro_global_badge, roGlobal);
