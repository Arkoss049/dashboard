(function () {
  ...
  function renderTable(list) {
    ...
    list.forEach((p, index) => {
      ...
      const statusSelect = `
        <select class="status-select status-${(p.status || 'A contacter').toLowerCase().replace(/ /g, '-')}" data-index="${index}">
      ...
      tr.innerHTML = `
        ...
        <td>${p.lastUpdate || ''}</td>
        <td>
          <button class="btn btn-ghost btn-notes" data-index="${index}">
            ${notesIcon}
          </button>
        </td>
        <td>
          <button class="btn btn-ghost btn-edit" data-index="${index}">✏️</button>
    ...
    document.querySelectorAll('#prospectTableBody .status-select').forEach((select) => {
      select.addEventListener('change', (e) => {
        ...
        prospects[index].status = e.target.value;
  ...
  window.filterAndSortProspects = function () {
    ...
    switch (sortValue) {
      ...
      case 'date_asc':
        filtered.sort((a, b) => {
          const da = new Date(a.lastUpdate?.split('/').reverse().join('-') || 0);
          const db = new Date(b.lastUpdate?.split('/').reverse().join('-') || 0);
          return da - db;
        });
        break;
      case 'date_desc':
